...
class scDCC(nn.Module):
    def __init__(self, input_dim, z_dim, n_clusters, encodeLayer=[], decodeLayer=[], 
        ...
        self.mu = Parameter(torch.Tensor(n_clusters, 32))
    ...
    def pretrain_autoencoder(self, x, X_raw, size_factor, batch_size=256, lr=0.001, epochs=400, ae_save=True, ae_weights='AE_weights.pth.tar'):
        ...
        for epoch in range(epochs):
            for batch_idx, (x_batch, x_raw_batch, sf_batch) in enumerate(dataloader):
                ...
                x_raw_tensor = x_raw_batch.cuda()
                sf_tensor = sf_batch.cuda()
    ...
    def save_checkpoint(self, state, index, filename):
        newfilename = os.path.join(filename, '512_checkpoint_%d.pth.tar' % index)
    ...
    def fit(self, X, X_raw, sf, ml_ind1=np.array([]), ml_ind2=np.array([]), cl_ind1=np.array([]), cl_ind2=np.array([]), 
        ...
        self.y_pred = kmeans.fit_predict(data.detach().cpu().numpy())
        self.y_pred_last = self.y_pred
        # 使用detach()并对其值进行了修改，一定要保证不需要使用到mu的梯度信息。此处获取聚类中心点，用于求q分布
        self.mu.detach().copy_(torch.Tensor(kmeans.cluster_centers_))
        if y is not None:
        ...
        for epoch in range(num_epochs):
            if epoch % update_interval == 0:
                ...
                self.y_pred = torch.argmax(q, dim=1).detach().cpu().numpy()
            ...
            train_loss = 0.0
            reconstrct_loss_val = 0.0
            cluster_loss_val = 0.0
            for batch_idx in range(num_batch):
                inputs = X[batch_idx*batch_size : min((batch_idx+1)*batch_size, num)]
                rawinputs = X_raw[batch_idx*batch_size : min((batch_idx+1)*batch_size, num)]
                sfinputs = sf[batch_idx*batch_size : min((batch_idx+1)*batch_size, num)]
                ...
                reconstrct_loss = self.zinb_loss(rawinputs, meanbatch, dispbatch, pibatch, sfinputs)
                loss = cluster_loss + reconstrct_loss
                loss.backward()
                optimizer.step()
                cluster_loss_val += cluster_loss.detach() * len(inputs)
                reconstrct_loss_val += reconstrct_loss.detach() * len(inputs)
                train_loss = cluster_loss_val + reconstrct_loss_val

            print("#Epoch %3d: Total: %.4f Clustering Loss: %.4f ZINB Loss: %.4f" % (
                epoch + 1, train_loss / num, cluster_loss_val / num, reconstrct_loss_val / num))
